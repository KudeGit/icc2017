...
We consider a typical enterprise or campus wireless access network.
Usually a single Cisco Wireless Controller (WLC) as the Cisco 8540 WLC or the Cisco Catalyst 3850 can support
thousand of Access Points (AP) and ten of thousands of clients~\cite{web:WLC-datasheets-site}.
In these network deplyoments, to simplify configurations and management operations, usually lightweight APs are used~\cite{rfc:RFC-5412}.
In this scenario, APs can not act independently of a WLC and offers wirelless connectivity to the clients.
The WLC has a global view of the network as it control and manage all the operations and client connections.
For these reasons, WLC controller should provide enough information to monitor and troubleshoot the network
and is the first device network administrators check when problems arise.
A common source of information are show commands.
However, show commands might not be effective to detect seldom issues or might lead to a low response time,
since they only offer an aggregated view of some network statistics and are generated on demand.
Because of these reasons, we do not use show commands output, but we instead collect network logs generated by the WLC.
Indeed, logs are event triggered and they a offer a higher granularity than show commands.
...
\EndFor
\State minSup $\gets$ minSup $ø 10\% \times$ size(D)
\EndWhile
\State	return $S$
\EndProcedure
\end{algorithmic}
